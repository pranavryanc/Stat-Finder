  ...
  return (
    <div className="grid gap-3 border-b border-white/7 py-4 last:border-0 md:grid-cols-[1fr_170px_110px_110px] md:items-center md:py-3">
      ...
      <div className="grid grid-cols-2 gap-3 md:contents">
        <input type="number" inputMode="decimal" step="any" disabled={disabled} value={condition.value ?? ''} onChange={e => onChange({...condition, value:e.target.value === '' ? undefined : Number(e.target.value)})} placeholder={condition.operator === 'between' ? 'Min' : 'Value'} className="min-h-11 min-w-0 w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2.5 text-base outline-none disabled:cursor-not-allowed disabled:opacity-35 focus:border-cyan-400/60 md:text-sm" />
        <input type="number" inputMode="decimal" step="any" disabled={condition.operator !== 'between'} value={condition.secondValue ?? ''} onChange={e => onChange({...condition, secondValue:e.target.value === '' ? undefined : Number(e.target.value)})} placeholder="Max" className="min-h-11 min-w-0 w-full rounded-xl border border-white/10 bg-slate-950/70 px-3 py-2.5 text-base outline-none disabled:cursor-not-allowed disabled:opacity-20 focus:border-cyan-400/60 md:text-sm" />
      </div>
    </div>
  )
}
